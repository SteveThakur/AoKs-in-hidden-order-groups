...
We will need the next two lemmas repeatedly in the subsequent protocols. We briefly explain the motivation for these lemmas here. As before, for a set $\mc{S}$ of rational primes, we denote by $\bz_{\mc{S}}$ the localization \vs $$\bz_{\mc{S}}:= \;\Big\{\frac{a}{\pl_{p\in \mc{S}} p^{e_p}}\;:\;a\in\bz,\; e_p\in \bz \Big\} $$ of $\bz$ at all primes in $\mc{S}$. This is a principal ideal domain and, in particular, is integrally closed. 

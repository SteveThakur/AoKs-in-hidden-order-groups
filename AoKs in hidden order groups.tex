...
We now define soundness and knowledge extraction for our protocols. The adversary is modeled as two algorithms $\A_0$ and $\A_1$, where $\A_0$ outputs the instance $x \in X$ after $\ttt{PGen}$ is run, and $\A_1$ runs the interactive protocol with the verifier using a state output by $\A_0$. In slight deviation from the soundness definition used in
...
the fact that in the computationally-sound setting the instance itself may encode a trapdoor of the common reference string, which can enable the adversary to fool a verifier. Requiring that an effcient adversary outputs the instance $x$ prevents this. In our soundness definition the adversary $\A_1$ succeeds
...
\begin{Def} \normalfont \textbf{(Argument of Knowledge):} We say an argument system $(\ttt{PGen}, \mP, \V)$ is $\mP$ cannot forge a fake proof except with negligible probability. \end{Def}
...
The Fiat-Shamir heuristic [FS87] can be used to transform public coin argument systems to non-interactive systems.
...
Similarly, for two elements, $\al,\be$, we write \vs $$\al = \al_1{\al_2}^{-1}\;,\; \be = {\be_1}{\be_2}^{-1}\;\;,\;\;\al_1,\be_1\in\bz\;,\;\al_2,\be_2\in \bz\cap\bzs^{\times}.$$ We define $\GCD_{\mc{S}}(\al,\be):= \GCD(\al_1,\be_1)$. Thus, $\GCD_{\mc{S}}(\al,\be)$ is the unique non-negative integer that has no prime divisors in $\mc{S}$ and generates the ideal $\bzs \al+\bzs\be\sub \bzs$.\vspace{0.1cm}
...
\begin{prf} As before, the algorithm $\mc{A}$ extracts integers $\al_j\;(j=1,\cdots,k)$, $\be_{i,j}\;(i=1,\cdots, n\;,\;j= 1,\cdots,k)$ such that \vs $$a = \pl_{j=1}^k g_j^{\al_j}\;\;,\;\; b_i = \pl_{j=1}^k g_j^{\be_{i,j}},\;\;\;\;(\al_i,\be_{i,j}\;\in\;\bz)$$ where $g_1,\cdots,g_k$ are the $\bG$-elements obtained from queries to the oracle $\mc{O}_1$. Now, for a $\lam$-bit challenge $\gamma$, $\mc{A}$ generates $Q\in\bG$, $r\in [\ell]$ such that $Q^{\ell}a^r = \pl_{i=1}^n b_i^{\gamma^i}. $ By the preceding lemma, $\mc{A}$ can generate a rational $\wti{d}$ such that $a^{\wti{d}} = \pl_{i=1}^n b_i^{\gamma^i}.$ Since the event DLOG occurs with at most \np, it follows that $ \wti{d} = \slim_{i=1}^n \frac{\be_{i,j}}{\al_j}\gamma^i$ for $j = 1,\cdots, k$. Since $\gamma$ is randomly generated, it follows that with \op, we have  \vs $$\frac{\be_{i,1}}{\al_{1}} = \cdots =  \frac{\be_{i,n}}{\al_{n}}.$$ Setting $d_i:= \frac{\be_{i,1}}{\al_{1}}$, we have $a^{d_i} = b_i\;\forall\; i$.\end{prf}
...
\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item The Prover $\mc{P}$ computes \vs $${d}:= \pl_{i=1}^m d_m\;\;,\;\; {e}:= \pl_{j=1}^n e_n.$$ \vs

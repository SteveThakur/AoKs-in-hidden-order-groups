...
\noin 2. $\mP$ generates non-interactive proof for $\pr{\ttt{AggKE-1}}[A_{\tt{\cap}},\;(A_1,\cdots,A_n)]$, $\ttt{PoKE}[g,\;A_{\tt{\cap}}]$ and sends them to the Verifier $\V$.
...
\noin 6. $\mP$ computes $A_0:= A_{\tt{\cap}}^{e}$ and sends it to $\V$ along with a non-interactive proof for \\ $\ttt{PoGCD}[(A_{\tt{\cap}}, \pl_{i=1}^n A_i^{\gamma^i}),\;(A_{\tt{\cap}},A),\;(A_{\tt{\cap}},A_0)]$.

\noin 7. $\mP$ computes $\wti{A}:= A_{\tt{\cap}}^{\slim_{i=1}^n d_i^{n\lam}\gamma^i}$ and sends $\wti{A}$ to $\V$ along with a non-interactive proof for \\ $\ttt{MultPolyDLog}[A_{\tt{\cap}},\;(A_1,\cdots,A_n, \wti{A}),\;\wti{f}]$ where \vs $$f(X_1,\cdots,X_{n+1}):= \big(\slim_{i=1}^n \gamma^i X_i^{n\lam}\big) - X_{n+1}.$$

\noin 8. $\mP$ computes $A_{\tt{\cap}}^{e^{n\lam}}, A_{\tt{\cap}}^{e^{n\lam-1}}$ and sends $A_{\tt{\cap}}^{e^{n\lam}}, A_{\tt{\cap}}^{e^{n\lam-1}}$ to $\V$ along with non-interactive proofs for $$\ttt{PolyDLog}[(A_{\tt{\cap}}, A_0),\;(A_{\tt{\cap}}, A_{\tt{\cap}}^{e^{n\lam}}),\;X^{n\lam}]\;,\;\ttt{PolyDLog}[(A_{\tt{\cap}}, A_0),\;(A_{\tt{\cap}}, A_{\tt{\cap}}^{e^{n\lam-1}}),\;X^{n\lam-1}].$$
 
\noin 9. $\mP$ computes $\wti{A}_0 := A_{\tt{\cap}}^{\wti{e}}$ and sends $\wti{A}_0$ to $\V$ along with a non-interactive proof for \\ $\ttt{PoGCD}[(A_{\tt{\cap}}, \wti{A}),\;(A_{\tt{\cap}},A_{\tt{\cap}}^{e^{n\lam}}),\;(A_{\tt{\cap}},\wti{A}_0)]$.

\noin 10. $\mP$ generates a non-interactive proof for $\ttt{PoKE}[\wti{A}_0,\; A_{\tt{\cap}}^{e^{n\lam-1}}] .$
...
\noin 2. The Prover $\mP$ computes the integers \vs $$\weck{d}_i:= \frac{\Pi(\bigcup\limits_{i=1}^n \mc{M}_i)}{\Pi(\mcM_i)}.$$


\noin 3. $\mP$ samples a $\lam$-bit prime $\gamma$ and computes \vs $$e:= \GCD(\weck{d}_1, \slim_{i=1}^n d_i\gamma^i) .$$

\noin 4. $\mP$ computes $$\wti{e}:= \GCD(e^{n\lam}, \slim_{i=1}^n \weck{d}_i^{n\lam}\gamma^i) .$$ If $\wti{e}$ does not divide $e^{n\lam-1}$, $\mP$ returns to Step 3 and samples another $\gamma$.
...
\noin 6. $\mP$ computes $A_0:= g^{e}$ and sends $A_0$ to $\V$ along with a non-interactive proof for\\ $\ttt{PoGCD}[(g, \weck{A}),\;(g, A_{\tt{\cup}}),\;(g, A_0)].$

\noin 7. $\mP$ computes $\wti{A}:= g^{\slim_{i=1}^n \weck{d}_i^{n\lam}\gamma^i}$ and sends $\wti{A}$ to $\V$ along with a non-interactive proof for $\ttt{MultPolyDLog}[g, \;(A_1,\cdots,A_n, A_{\tt{\cup}}, \wti{A}), \wti{f}]$ where \vs $$\wti{f}(X_1,\cdots,X_{n+2}):= \Big( X_{n+1}^{n\lam}\cdot \slim_{i=1}^n \frac{\gamma^i}{X_i^{n\lam}} \Big)- X_{n+2}. $$
...
\[
  \mc{R}_{{\protect\verb|MultPolyDLog|}}[a, (b_1,\cdots, b_n), (f_1,\cdots,f_k)] = \left\{\begin{array}{l}
    \big(a\in\mb{G},\; (b_1,\cdots, b_n)\in\mb{G}^n);\\
    f_1,\cdots,f_k\in\bq(X_1,\cdots,X_n)\big);\\ 
    (d_1,\cdots,d_n)\in \bz^n)\;: \\
    b_i = a^{d_i}\;\forall\; i\;\bigwedge \\   \bigwedge\limits_{i=1}^k f_i(d_1,\cdots,d_n) = 0 
  \end{array}\right\}
\]
...
\noin 14. Protocol for the intersection of multisets \vspace{-0.7cm}

\[
  \mc{R}_{\ttt{int}}[g, (A_1,\cdots,A_n), A_{\tt{\cap}}] = \left\{\begin{array}{l}
    (g\in\mb{G};\;\\
     (A_1,\cdots,A_n, A_{\tt{\cap}})\in\mb{G}^{n+1});\\
...
\noin 15. Protocol for the union of multisets \vspace{-0.7cm}

\[
  \mc{R}_{\ttt{uni}}[g, (A_1,\cdots,A_n), A_{\tt{\cup}}] = \left\{\begin{array}{l}
    (g\in\mb{G};\;\\
     (A_1,\cdots,A_n, A_{\tt{\cup}})\in\mb{G}^{n+1});\\

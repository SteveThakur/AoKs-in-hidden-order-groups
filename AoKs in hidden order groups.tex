...
\noindent 2. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lamb$-bit prime $\ell$.
...
\noindent 3. $\mc{P}$ computes the integers $q_1$, $q_2$, $r_1$, $r_2$ such that \vs $$d_1 = q_1\cdot\ell+r_1\;,\; d_2 = q_2\cdot\ell+r_2\;,\;\; r_1,r_2\in [\ell].$$
...
\noindent 5. $\V$ verifies that $r_1\in [\ell]$ and computes $r_2:= f(r_1)\Mod{\ell}$.
...
\noindent 3. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit prime $\ell\not\equiv 1\Mod{p}$.
 
\noindent 4. $\mc{P}$ computes the integers $r_i:=d_i\Mod{\ell}\;\; (i=1,\cdots, n)$ and the integers $\wti{q}$, $\wti{r}$ such that \vs $$\slim_{i=1}^n d_i^{p}\gamma^i = \wti{q} \cdot \ell+\wti{r}\;\;,\:\;\wti{r}\in [\ell] $$ and sends $(r_1,\cdots,r_n)\in [\ell]^n$ to $V$.
...
\noindent 9. $\mc{V}$ verifies the equations \vs $$ (Q_0)^{\ell} a^{r_{0}} \sta b_0 \;\;\bigwedge\;\; (\weck{g})^{\ell}g^{\wti{r}} \stackrel{?}{=}\wti{g} \;\;\bigwedge\;\;\Big( \bigwedge\limits_{j=1}^k  f_j(r_1,\cdots,r_n) \stackrel{?}{\equiv} 0\Mod{\ell} \Big)$$ and accepts the validity of the claim if and only if all equations hold.\qed \end{enumerate}
...
\noindent 2. $\mc{P}$ uses the Euclidean algorithm to compute integers $e_1, e_2$ such that $e_1d_1 + e_2d_2 = 1$.
...
%\noindent \textbf{Input:} $\mc{P}$ possesses

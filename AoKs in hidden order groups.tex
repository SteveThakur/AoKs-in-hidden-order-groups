...
\begin{prf} By the element representation of [Sho97] (see the Appendix of [BBF19] for additional details), every element $\mc{A}$ obtains in response to an $\mc{O}_2$ query is expressible as a linear combination $\pl_{i=1}^k g_i^{x_i}$ where the $g_1,\cdots,g_k$ are outputs of $\mc{O}_1$ and the $x_i$ are integers. 
...
In the special case where $a = g$, the subprotocol of $\ttt{PoAggKE-1}$ or $\ttt{PoMultPolyDLog}$ where the Prover computes the element \vs $$\wti{g}:= g^{\slim_{i=1}^n d_i^{p}\gamma^i}$$ and sends it to the Verifier is redundant. So the proof would be a bit smaller and the Prover's computational burden would be substantially lower in this special case. \vspace{0.2cm}
...
\subsection{\fontsize{11}{11} Protocols for disjointness of multisets in an accumulator} 

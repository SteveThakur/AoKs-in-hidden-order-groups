...
\begin{abstract} We study non-interactive arguments of knowledge (AoKs) in groups of hidden order. We provide protocols to aggregate the knowledge of the discrete logarithms between multiple elements and to prove certain relations between multiple discrete logarithms. In particular, we provide AoKs for disjointness of sets in cryptographic accumulators, with a view toward applications to sharded blockchains. 
...
\noin In the next section, we will generalize this protocol to multivariate polynomial relations for multiple discrete logarithms.
...
\begin{Prop} The protocol \verb|PoAggKE-1| is an argument of knowledge for the relation\\ $\mc{R}_{{\protect\verb|PoAggKE-1|}}$ in the generic group model.\end{Prop}
...
\begin{Prop} The protocol \verb|PoMultPolyDLog| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoMultPolyDLog|}}$ in the generic group model.\end{Prop}
...
\begin{Prop} The protocol \verb|PoAggKE-2| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoAggKE-2|}}$ in the generic group model.\end{Prop}
...
\begin{Prop} The protocol \verb|PoAggRelPrimeDLog-1| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoAggRelPrimeDLog-1|}}$ in the generic group model.\end{Prop}
...
\begin{Prop} The Protocol \verb|AggRelPrimeDLog-2| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoAggRelPrimeDLog-2|}}$ in the generic group
model.\end{Prop}
...
\begin{Prop} The protocol \verb|PoAggRelPrimeDLog-3| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoAggRelPrimeDLog-3|}}$ in the generic group model.\end{Prop}
...
\noindent \textbf{Inputs:} Elements $B, C\in \mb{G}$,\;  $\mc{B} = \{b_1,\cdots, b_m\}$, $\mc{C} = \{c_1,\cdots, c_n\}$ of $\mb{G}$
...
\begin{Prop} The protocol \verb|PoAggRelPrimeDLog-4| is an argument of knowledge for the relation $\mc{R}_{{\protect\verb|PoAggRelPrimeDLog-4|}}$ in the generic group model.\end{Prop}
...
\subsection{\fontsize{11}{11} Applications}

\noin \textbf{Sharded blockchains:} Consider the setting of a stateless sharded blockchain that, instead of a Merkle tree, hinges on a cryptographic accumulator instantiated with a hidden order group $\mb{G}$. Let $g$ be a randomly selected element of $\mb{G}$ and $S_1,\cdots, S_n$ the distinct shards. Let $\mc{D}_i$ denote the data in shard $S_i$. Then the accumulated digest (the analog of the Merkle root hash) of $S_i$ is given by \vs $$A_i:= g^{\pl_{d\in\mc{D}_i} d}.$$  

\noin 1. In order to demonstrate that the data sets are disjoint, a Prover (such as a miner or an untrusted server) can provide a proof for the relation $\mc{R}_{{\protect\verb|AggRelPrimeDLog-1|}}[a, \mc{A}]$ where $\mc{A} = (A_1,\cdots,A_n)$.\vspace{0.2cm}


Consider a single shard with accumulated digest $A$. Let $\V_1,\cdots,\V_n$ be verifiers (such as light nodes) on the network. Let $\mc{D}_i$ denote the data set corresponding to $\V_i$. A Prover $\mP$ (such as a miner or an untrusted server) can prove to the nodes $\V_i$ that their data sets are disjoint as follows.

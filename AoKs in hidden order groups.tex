...
\noin If the challenges are public coin, any interactive argument of knowledge can be converted into a non-interactive argument using a Fiat-Shamir heuristic ([FS86]).
...
\begin{Lem}\label{ram} Let $p$ be a prime and let $f(X)$ be a univariate degree $n$ polynomial in $\bz[X]$ with its leading coefficient not divisible by $p$. For a randomly generated integer $\gamma$, the probability that $f(\gamma)\equiv 0\Mod{p^{n\lam}}$ is negligible. \end{Lem}

\begin{prf} Let $F$ be a splitting field of $f(X)$, $\mc{O}_F$ its ring of integers and let \vs $${f}(X) = c\cdot \pl_{i=1}^n (X-\al_i)$$ be the factorization of ${f}(X)$ over $F$, where $c\in\bz$ is the leading coefficient of $f(X)$. Let $\mfp_{1},\cdots,\mfp_g$ be the distinct primes of $F$ lying over $p$. Since the extension $F/\bq$ is Galois, we have \vs $$p\mc{O}_F = \pl_{i=1}^{g} \mfp_i^e = \bigcap\limits_{i=1}^{g} \mfp_i^e$$ where $e\geq 1$ is the ramification degree and the Galois group $\Gal(F/\bq)$ acts transitively on the set $\{ \mfp_1,\cdots,\mfp_g\}$.
...
%The adaptive root assumption implies that the the probability of this event is negligible. The next lemma follows directly from the techniques in the appendix of [BBF19]. We provide a proof for the reader's convenience.
...
%\noin 3. $\mc{P}$ computes \vs $$b:= \prod\limits_{i=1}^n b_i^{\gamma^i} \in \mb{G}.$$
...
\noin 2. $\mc{P}$ computes \vs $$p:= \ttt{NextPrime}(n\lam)\;\;,\;\;\wti{g} :=g^{\sum\limits_{i=1}^n d_i^{p}\gamma^i}\vs $$ and sends $\wti{g}$ to the Verifier $\mc{V}$.
...
\noin 3. $\mc{P}$ computes $$p:= \ttt{NextPrime}(n\lam)\;\;,\;\;\wti{g} := g^{\slim_{i=1}^n d_i^{p}\gamma^i}\vs $$ and sends it to the Verifier $\mc{V}$. 
...
\noin 6. $\mc{P}$ computes \vs $$p:= \ttt{NextPrime}(n\lam)\;\;,\;\;\wti{g} := g^{\slim_{i=1}^n d_i^{p}\gamma^i}\vs $$ and sends $\wti{g}$ to $\mc{V}$.
...
\noin 2. $\mP$ computes the elements \vs $$\wti{w}_1:= w_1^{\pl_{x\in\mc{D}} x^{\mr{mult}(\bigcup\limits_{i=2}^n \mc{M}_i, x)}}\;\;,\;\; \wti{w}_1^{\Pi(\mc{D})}\;\in\;\mb{G}\vs $$ and sends them to $\V$ along with a non-interactive $\ttt{PoE}[\wti{w}_1,\;\Pi(\mc{D}),\;\wti{w}_1^{\Pi(\mc{D})}]$. \vs

\noin 3. $\mP$ generates a non-interactive proof for \verb|AggKE-1|$[\weck{w}_1,\;(w_2,\cdots,w_n)]$ and sends it to $\V$. \vs
...
\noin 2. $\mP$ computes \vs $$\what{A}:= \what{w}_1^{\pl_{x\in\mc{D}}x^{\mr{mult}(\mc{M}_1,x)}}\;,\;\what{A}^{\Pi(\mc{D})}\;\in\mb{G}\vs $$ and sends them to $\V$ along with a non-interactive \verb|PoE| for the exponentiation $\what{A}^{\Pi(\mc{D})}$. \vs
...
\noin \textbf{Input:} Commitments $A_i := g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\tt{\cap}}\in\mb{G}$

\noin \textbf{Claim:} $A_{\tt{\cap}} = \protect{\verb|Com|}(g, {\bigcap\limits_{i=1}^n} \mc{M}_i) := g^{\Pi({\bigcap\limits_{i=1}^n} \mc{M}_i)}$.

\begin{prf1} \normalfont \noin 1. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit challenge $\gammma$.

\noin 2. The Prover computes \vs $$g_1:= g^{\slim{i=1}^n} d_i\gamma^i\;,\;g_2:= g^{\slim{i=1}^n} e_i\gamma^i\vs $$ and sends them to the Verifier $\V$.
...
\noin 7. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit challenge $\what{\gamma}$.
...
\noin 4. $\mP$ generates a non-interactive proof for $\protect{\tt{MultPolyDLog} }[g,\;(A_1,\cdots,A_n, \weck{A}_1,\cdots,\weck{A}_n, A_{\tt{\cap}}),\;f]$ where \vs $$f(X_1,\cdots,X_{2n+1}):= (\slim_{i=1}^n X_iX_{n+i})-X_{2n+1} \vs $$ and sends it to $\V$.
...
\noin \textbf{Input:} Commitments $A_i := g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\tt{\cup}}\in\mb{G}$
...
\noin 3. $\mP$ computes \vs $$\wti{A}:= g^{\pl_{i=1}^n d_i} = \ttt{Com}(g, \slim_{i=1}^n \mcM_i)\vs $$ and sends $\wti{A}$ to the Verifier $\V$ along with a non-interactive proof for the relation \\ $\ttt{MultPolyDLog}[g,\;(A_1,\cdots,A_n,\wti{A}),\;f]$ where \vs $$f(X_1,\cdots,X_{n+1}):= (\pl_{i=1}^n X_i)-X_{n+1}. $$
...
\noin \textbf{Input:} Commitments $A_i = \ttt{Com}(g, \mc{M}_i) := g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\tt{\cup}}\in\mb{G}$
...
\noin \textbf{Inputs :} $a_1,a_2\in\mb{G}$  \;,\;$(b_1,\cdots, b_n), \;\;(c_1,\cdots, c_n)\;\in\; \mb{G}^n$.

%\noin \textbf{Input:} $\mc{P}$ possesses

\noin \textbf{Claim:} The Prover possesses integers $d_1,\cdots, d_n$ such that $a_1^{d_i} = b_i,\; a_2^{d_i} = c_i$.

\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit prime $\gamma$.
...
\noin 7. $\mc{P}$ generates a non-interactive proof for \verb|EqDLog|$[(a_1, B),\; (a_2, C)]$ and sends it to $\mc{V}$.

\noin 8. The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit prime $\gamma_{0}$.
...
\noin 10. $\V$ verifies that $(r_1,\cdots,r_n)\in [\ell]^n$ and computes \vs $$\wti{r}:= \slim_{i=1}^n r_i^{p}\gamma^i \Mod{\ell}\;,\;r:= \slim_{i=1}^n r_i\gamma^i \Mod{\ell}\;,\;r_0:= \slim_{i=1}^n r_i\gamma_{0}^i \Mod{\ell}.$$
...
\noin We now show how to generalize the last protocol for multiple discrete logarithms while keeping the communication complexity constant-sized and independent of the number of discrete logarithms. We call the following protocol the \textit{Aggregated Equality of Discrete Logarithms} or \verb|AggEqDLog| for short. We provide an argument of knowledge for  the following relation:
...
\noin \textbf{Inputs :} $a,b\in\mb{G}$, $(a_1, \cdots, a_n)\in\mb{G}^n$, $(b_1, \cdots, b_n)\in\mb{G}^n$.

%\noin  

\noin \textbf{Claim:} The Prover possesses an integer $d$ such that $a^d = b$ and $a_i^d = b_i$ for $i = 1,\cdots, n$ .

\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item The hashing algorithm $\ttt{H}_{\ttt{FS},\lam}$ generates a $\lam$-bit integer $\gamma$.

\noin 2. The Prover $\mc{P}$ computes the elements \vspace{-0.15cm}$$\wti{a}:= \prod\limits_{i=1}^n a_i^{\gamma^i}, \;\;\wti{b}:= \prod\limits_{i=1}^n b_i^{\gamma^i}\;\; \in \mb{G}.$$

\noin 3. $\mc{P}$ generates a non-interactive proof for \verb|EqDLog|$[(a, b),\; (\wti{a}, \wti{b})]$ and sends it to the Verifier $\mc{V}$.

\noin 4. $\mc{V}$ independently computes the elements $\wti{a}, \wti{b}$ and accepts if and only if the proof for\\ \verb|EqDLog|$[(a, b),\; (\wti{a}, \wti{b})]$ is valid.\qed \end{enumerate}
\vspace{0.1cm}

...
\noindent 3. $\mc{P}$ generates non-interactive proofs for \verb|MultPolyDLog|$[a_1,\;(b_1,\cdots,b_m, B),\; (\pl_{i=1}^m X_i) -X_{m+1}]$, \verb|MultPolyDLog|$[a_2,\;(c_1,\cdots,c_n, C),\; (\pl_{j=1}^n X_j) -X_{n+1}]$ and sends them to $\mc{V}$.

\noindent 4. $\mc{P}$ generates non-interactive proofs for 
...
\noin 4. $\mP$ generates a non-interactive proof for \vs $$\protect{\tt{MultPolyDLog} }[g,\;(A_1,\cdots,A_n, \weck{A}_1,\cdots,\weck{A}_n, A_{\mr{int}}),\;(\slim_{i=1}^n X_iX_{n+i})-X_{2n+1}] $$ and sends it to $\V$.
...
\noin \textbf{Claim:} $A_{\mr{uni}} = \protect{\verb|Com|}(g, {\bigcup\limits_{i=1}^n} \mc{M}_i) := g^{\Pi({\bigcap\limits_{i=1}^n} \mc{M}_i)}$.
...
\noin 2. $\mP$ computes \vs $$\wti{A}:= g^{\pl_{i=1}^n d_i} = \ttt{Com}(g, \slim_{i=1}^n \mcM_i)$$ and sends $\wti{A}$ to the Verifier $\V$ along with a non-interactive proof for \vs $$\ttt{MultPolyDLog}[g,\;(A_1,\cdots,A_n,\wti{A}),\;(\pl_{i=1}^n X_i)-X_{n+1}].$$

\noin 3. $\mP$ generates a non-interactive proof for $\pr{\ttt{AggKE-2}}[(A_1,\cdots,A_n),\;A_{\uni}] $ and sends it to $\V$.

\noin 4. $\mP$ computes $\weck{A}_i:= g^{\what{e}_i}\;(i=1,\cdots,n) $ and sends $\weck{A}_1,\cdots,\weck{A}_n$ to $\V$ along with a non-interactive proof for \vs $$\pr{\ttt{MultPolyDLog}}[g,\;(A_1,\cdots,A_n, \weck{A}_1,\cdots,\weck{A}_n, A_{\uni}, \wti{A}),\;X_{2n+1}\pl_{i=1}^n X_i(\slim_{j=1}^n \frac{X_{n+j}}{X_j}) - X_{2n+2}]. $$
...
As before, let $\mc{M}_1,\cdots, \mc{M}_n$ be multisets a client $\V$ outsources to a server node $\mP$. The client stores succinct commitments \vs $$\tt{Com}(g, \mc{M}_i) = g^{\Pi(\mc{M}_i)},$$ where $g$ is a randomly generated element of $\bG$. The multisets are dynamic and hence, the commitments need to be updated in response to changes. \vspace{0.15cm}

...
assumptions are believed to hold in generic groups of unknown order [DK02, BBF19, DGS20].
...
\begin{Def} \normalfont \textbf{Completeness:} We say an argument system $(\ttt{PGen}, \mP, \V)$ for a relation $\mc{R}$ is complete if for all $(x,w)\in \mc{R}$, \vs $$\mbf{Prob}\big(\langle \V(\ttt{pp}, x), \mP(\ttt{pp}, w)\rangle\big) =1\;:\; \ttt{pp} \xleftarrow{\$} \ttt{PGen}(\lam) \big)\; = \; 1. $$ \end{Def}

\begin{Def} \normalfont \textbf{Soundness:} We say an argument system $(\ttt{PGen}, \mP, \V)$ is \textbf{sound} if $\mP$ cannot forge a fake proof except with negligible probability. \end{Def}

\begin{Def} \normalfont \textbf{Argument of Knowledge :} We say an argument system is an argument of knowledge if for any polynomial time adversary $\A$, there exists an extractor $\E$ with access to $\A$'s internal state that can, with \op, extract a valid witness whenever $\A$ is convincing. \end{Def}


\begin{Def} \normalfont A \textbf{non-interactive} argument system is an argument system where the interaction between $\mP$ and $\V$ consists of only a
single round.\end{Def}

The Fiat-Shamir heuristic [FS87] can be used to transform public coin argument systems to non-interactive systems. Instead of the Verifier generating the challenges, this function is performed by a public hashing algorithm agreed upon in advance.
...
\noindent \textbf{The RootFactor Algorithm:} Given elements $a, A\in\mb{G}$ and integers $d_1,\cdots, d_n$, $D$ such that \vs $$D= \pl_{i=1}^n d_i\; ,\; a^{D} = A,$$ the {\tt{RootFactor}} algorithm ([BBF19], [STY01]) allows us to compute elements $a_1,\cdots, a_n$ such that \vs $$a_1^{d_1}=\cdots = a_n^{d_n} = A$$ in runtime $\mbf{O}(\log(D)\log(\log(D)))$. Na\"ively, this would take runtime $\mbf{O}(\log^2(D))$, which would be impractical for many applications.
...
\noin Note that if there exists an element $a\in\mb{G}$ and distinct rationals $\frac{d_1}{d_2}, \frac{d_3}{d_4}\;(d_i\in\bz)$ such that \vs $$a^{{d_1}{d_2}^{-1}} = a^{{d_3}{d_4}^{-1}},$$ then $a^{d_1d_4-d_2d_3} = 1$ and $d_1d_4-d_2d_3\neq 0$. So the low order assumption implies that a PPT algorithm cannot generate such a tuple $(a,d_1,d_2,d_3,d_4)$ except with negligible probability.\\ Furthermore, by Shamir's trick, the condition is equivalent to the Prover $\mP$ being able to generate an element $a_0\in\mb{G}$ and co-prime integers $d_1,d_2$ such that \vs $$\al = \frac{d_1}{d_2}\;\;,\;\;a_0^{d_2} =a\;,\; a_0^{d_1} =b\;,\; $$
...
\begin{Def} For elements $a, b$ in a hidden order group $\mb{G}$, we say $ a \equiv_{\lam} b$ with respect to a Prover $\mc{P}$ if $\mc{P}$ verifiably possesses  relatively prime $\lamb$-smooth integers $d_1, d_2$ such that $a^{d_1} = b^{d_2}.$\end{Def}
...
\begin{Lem}$\mr{(i)}$. For rationals $d_1,\cdots, d_n\in \bq$ and a randomly generated $\lam$-bit integer $\gamma$, if \vs$$\sum\limits_{i=1}^n d_i\gamma^i\; \in \;\bzlam,$$ with \op, $(d_1,\cdots, d_n)\in \bzlam^n$.\vspace{0.1cm}

\noin $\mr{(ii)}$. Let $k$ be any integer $\geq n\lam$. Let $\mc{S}$ be a set of rational primes and let $\bz_{\mc{S}}$ be the localization of $\bz$ at all primes in $S$. For rationals $d_1,\cdots, d_n\in \bq$ and a randomly generated $\lam$-bit integer $\gamma$, if \vs $$\slim_{i=1}^n d_i^{k}\gamma^i\; \in \;\bz_{\mc{S}},$$ with \op, $(d_1,\cdots, d_n)\in \bz_{\mc{S}}^n$.
\end{Lem}
...
\noin Then there exists an extractor \hspace{0.05cm}$\E$ that can, with \op, extract integers $d_1, d_2$ such that \vs $$a^{d_1} = b^{d_2}\;\wedge\;d_1\equiv rd_2\Mod{\ell}.$$\end{Lem}

\begin{prf} By the element representation lemma of [Sho97], for every element $\mc{A}$ obtains in response to an $\mc{O}_2$ query, an extractor $\E$ succeeds with overwhelming probability at the task of expressing the element as a  linear combination $\pl_{i=1}^k g_i^{x_i}$ where the $g_1,\cdots,g_k$ are responses to queries to oracle $\mc{O}_1$ and the $x_i$ are integers bounded by $\ttt{poly}(\lam)$. Let \vspace{-0.3cm} $$a = \pl_{i=1}^k g_i^{\al_i}\;\;,\;\; b = \pl_{i=1}^k g_i^{\be_i},\;\;\;\;(\al_i,\be_i\;\in\;\bz).$$ Now, for a randomly generated $\lam$-bit prime $\ell$, $\mc{A}$ is able to output $(Q, r)\; \in\;\bG \times [\ell]$ such that $Q^{\ell}a^r = b$. Since the event DLOG occurs with at most \np, we may assume \vs $${\be_1}{\al_1}^{-1}\equiv\cdots\equiv {\be_k}{\al_k}^{-1}\equiv r \Mod{l}.$$ Since the $\lam$-bit prime $\ell$ was randomly generated, this implies that with \op, \vs $${\be_1}{\al_1}^{-1} = \cdots =  {\be_k}{\al_k}^{-1}.$$ Thus, $a^{\be_1} = b^{\al_1}$. \end{prf}
\vspace{0.1cm}

\begin{comment}

\begin{Lem} Let $a,\; b_1,\cdots,b_n$ be elements of a generic hidden order group $\bG$. Let $\mc{A}$ be a generic PPT algorithm that succeeds at the following task:

\noin - The Fiat-Shamir heuristic generates a $\lam$-bit challenge $\gamma$ and a $\lam$-bit prime $\ell$.

\noin - $\mc{A}$ generates an element $Q\in \mb{G}$ and an integer $r\in [\ell]$ such that $Q^{\ell}a^r = \pl_{i=1}^n b_i^{\gamma^i}$.\vspace{0.1cm}

\noin Then there exists an extractor $\E$ that can, with \op, extract rationals $d_1,\cdots,d_n$ such that the numerators/denominators of the $d_i$ are bounded by $\tt{poly}(\lam)$ and \vs $$a^{d_i} = b_i \;\forall\;i
\;\;\bigwedge\;\;\slim_{i=1}^n d_i\gamma^i\equiv r\Mod{\ell}.$$ \end{Lem}

\begin{prf} As before, the extractor $\E$ extracts integers $\al_j\;(j=1,\cdots,k)$, $\be_{i,j}\;(i=1,\cdots, n\;,\;j= 1,\cdots,k)$ such that \vs $$a = \pl_{j=1}^k g_j^{\al_j}\;\;,\;\; b_i = \pl_{j=1}^k g_j^{\be_{i,j}},\;\;\;\;(\al_i,\;\be_{i,j}\;\in\;\bz)$$ where $g_1,\cdots,g_k$ are the $\bG$-elements obtained from queries to the oracle $\mc{O}_1$. Now, for a $\lam$-bit challenge $\gamma$, $\mc{A}$ generates $Q\in\bG$, $r\in [\ell]$ such that $Q^{\ell}a^r = \pl_{i=1}^n b_i^{\gamma^i}. $ By the preceding lemma, $\E$ can generate a rational $\wti{d}$ such that its numerator/denominator are bounded by $\ttt{poly}(\lam)$ and \vs $$\pl_{j=1}^k g_j^{\wti{d}\al_j} = a^{\wti{d}} = \pl_{i=1}^n b_i^{\gamma^i} = \pl_{j=1}^k g_j^{\slim_{i=1}^n \be_{i,j}\gamma^i}$$ Since the event DLOG occurs with at most \np, it follows that \vs $$ \wti{d} = \slim_{i=1}^n ({\be_{i,j}}{\al_j}^{-1})\gamma^i\;\;\;\text{ for } j = 1,\cdots, k.$$ Since $\gamma$ is randomly generated, it follows that with \op, \vs $$\be_{i,1}\al_{1}^{-1} = \cdots =  \be_{i,k}{\al_{n}}^{-1}.$$ Setting $d_i:= \be_{i,1}\al_{1}^{-1}$ yields $a^{d_i} = b_i\;\forall\; i$.\end{prf}
...
\noin Then there exists an extractor \hspace{0.05cm}$\E$ that can, with \op, extract rationals $d_1,\cdots,d_n$ such that the numerators/denominators of the $d_i$ are bounded by $2^{\tt{poly}(\lam)}$ and \vs $$a^{d_i} = b_i \;\forall\;i \;\;\bigwedge\;\; d_i\equiv r_i\Mod{\ell}\;\forall\;i.$$ \end{Lem}

\begin{prf} As before, the extractor $\E$ extracts integers $\al_j\;(j=1,\cdots,k)$, $\be_{i,j}\;(i=1,\cdots, n\;,\;j= 1,\cdots,k)$ such that \vs $$a = \pl_{j=1}^k g_j^{\al_j}\;\;,\;\; b_i = \pl_{j=1}^k g_j^{\be_{i,j}},\;\;\;\;(\al_i,\;\be_{i,j}\;\in\;\bz)$$ where $g_1,\cdots,g_k$ are the $\bG$-elements obtained from queries to the oracle $\mc{O}_1$. Now, in response to a $\lam$-bit challenge $\gamma$, $\mc{A}$ generates $Q\in\bG$ such that $Q^{\ell}a^r = \pl_{i=1}^n b_i^{\gamma^i}, $ where $r:= \slim_{r_i}{\gamma}^i\Mod{\ell}$. By the preceding lemma, $\E$ can generate a rational $\wti{d} = \wti{d}_1\wti{d}_2^{-1}$ ($\wti{d}_1, \wti{d}_2\in\bz$) with $\wti{d}_1, \wti{d}_2$ bounded by $2^{\tt{poly}(\lam)}$ and \vs $$\pl_{j=1}^k g_j^{\wti{d}\al_j} = a^{\wti{d}} = \pl_{i=1}^n b_i^{\gamma^i} = \pl_{j=1}^k g_j^{\slim_{i=1}^n \be_{i,j}\gamma^i}$$ Since the event DLOG occurs with at most \np, it follows that \vs $$ \wti{d} = \slim_{i=1}^n ({\be_{i,j}}{\al_j}^{-1})\gamma^i\;\;\;\text{ for } j = 1,\cdots, k.$$ Since $\gamma$ is randomly generated, it follows that with \op, \vs $$\be_{i,1}\al_{1}^{-1} = \cdots =  \be_{i,k}{\al_{n}}^{-1}.$$ Setting $d_i:= \be_{i,1}\al_{1}^{-1}$ yields $a^{d_i} = b_i\;\forall\; i$.

Now, if $\slim_{i=1}^n d_i\gamma^i \not\equiv r\Mod{\ell}$, the extractor $\E$ could efficiently extract a $\ell$-th root of $a$, which would violate the adaptive root assumption. Hence, with \op, \vs $$\slim_{i=1}^n d_i\gamma^i \equiv r\equiv \slim_{i=1}^n r_i\gamma^i \Mod{\ell}.$$ Since the $\lam$-bit integer $\gamma$ was randomly generated after the vector $(r_1,\cdots,r_n)$ was sent, the Schwart-Zippel lemma implies that with \op, $d_i\equiv r_i\Mod{\ell}\;\forall\; i$.\end{prf}
...
\begin{prf} We first show that the protocol is sound. We then show that an extractor can efficiently extract the integer $d$ by applying the Chinese remiander theorem to a sufficiently large (bounded by $\ttt{poly}(\lam)$) number of accepting transcripts.

Since the protocol \verb|PoKE| is secure ([BBF19]), the validity of the equations \vs $$Q_1^la_1^r \stackrel{?}{=}  b_1, \;\; Q_2^la_2^r \stackrel{?}{=} b_2, \;\; (\weck{g})^l g^r \stackrel{?}{=} \wti{g}$$ proves that $\mc{P}$ possesses integers $d_1, d_2$ such that $a_1^{d_1} = b_1$, $a_2^{d_2} = b_2$. Suppose, by way of contradiction, $d_1\neq d_2$. The adaptive root assumption implies that with \op,\vs $$d_1 \equiv r\equiv d_2 \Mod{\ell}.$$ But since the $\lamb$-bit prime $\ell$ is randomly generated, the integer $d_1-d_2$ is randomly and uniformly distributed modulo $\ell$ and hence, \vs $$ \Prob\big( d_1\equiv d_2\Mod{\ell}\; \Big|\; d_1\neq d_2    \big) = \frac{1}{\ell} =\negl(\lamb).$$ Thus, with overwhelming probability $d_1 = d_2$.

Now, an extractor $\E$ can simulate the extractor for $\ttt{PoKE}$ to obtain integers $d_1'$, $d_2'$ such that $a_1^{d'_1} = b_1$, $a_2^{d'_2} = b_2$. Since these integers satisfy $d_1'\equiv d_2'\Mod{\ell}$ for a randomly generated $\lam$-bit prime $\ell$, it follows that with \op, $d_1' = d_2'$.\end{prf}
...
\noin 5. The Fiat-Shamir heuristic generates a $\lam$-bit prime $\gamma_{0}$.

\noin 6. $\mP$ computes integers $q_0, r_0$ such that \vs $$\slim_{i=1}^n d_i\gamma_{0}^i = q_0\ell+r_0\;,\;r_0\in[\ell] .$$ He computes $Q_0:= a^{q_0}$ and sends it to $\V.$

\noindent 7. $\mc{V}$ verifies that $(r_1,\cdots,r_n)\in [\ell]^n$ and independently computes \vs $$b := \pl_{i=1}^n b_i^{\gamma^i}\;,\;b_0 := \pl_{i=1}^n b_i^{\gamma_{0}^i}\;\in \;\bG,$$ \vspace{-0.2cm} $$\wti{r}:= \slim_{i=1}^n r_i^{p}\gamma^i\Mod{\ell}\;,\;r:= \slim_{i=1}^n r_i\gamma^i\Mod{\ell}\;,\;r_0:=\slim_{i=1}^n r_i\gamma_{0}^i\Mod{\ell}.$$ 
...
When $a$ is not randomly generated, the most expensive part of the proof generation is computing the element $\wti{g}$. The effective runtime for this can be mitigated by the Prover pre-computing the set \vs $$\{g^{2^i}: 1\leq i \leq N \}$$ for some appropriately large integer $N$.

The Verifier's work can be reduced by the Prover sending Proofs of Exponentiation for the equations \vs $$b := \pl_{i=1}^n b_i^{\gamma^i}\;,\;b_0 := \pl_{i=1}^n b_i^{\gamma_{0}^i}\;,$$ instead of the Verifier independently performing these computations.
...
\begin{prf} (Sketch) We first show that the protocol is sound, i.e. if the Prover sends an accepting transcript, then with \op, he possesses integers $d_i$ such that $a^{d_i} = b_i\;\forall\;i$. We then show how an extractor $\E$ could extract the integers $d_i$ applying the Chinese remainder theorem to a sufficiently large number \big(bounded by $\ttt{poly}(\lam)$\big) of accepting transcripts. The low order assumption implies that except with \op, any PPT algorithm can generate at most a unique vector $(d_1,\cdots,d_n)\;\in\;\bz^n$ such that $a^{d_i} = b_i$.

Since the equation $Q_0^{\ell}a^{r_0} = b_0$ holds, lemma 2.5 implies that with \op, 
$\mP$ possesses rationals $d_1,\cdots,d_n$ such that \vs $$a^{d_i} = b_i \;\;,\;\;\slim_{i=1}^{n} d_i\gamma_{0}^i \equiv \slim_{i=1}^{n} r_i\gamma_{0}^i\Mod{\ell}.$$ Since the challenge $\gamma_{0}$ is randomly generated after the integers $r_1,\cdots,r_n$ have been sent, the Schwartz-Zippel lemma implies that with \op, $d_i\equiv r_i\Mod{\ell}\;\forall\;i$. 

Furthermore, we have $\wti{g} = (\weck{g})^{\ell}g^{\wti{r}},$ which implies that with \op, $\mP$ possesses a rational $\wti{d}$ such that \vs $$\wti{g} = g^{\wti{d}}\;\;,\;\;\wti{d}\equiv\wti{r}\equiv \slim_{i=1}^n r_i^p\gamma^i\equiv \slim_{i=1}^n d_i^p\gamma^i\Mod{\ell} .$$ The fractional root assumption then implies that with \op, $\wti{d}\in \bz$. Furthermore, since $\ell$ is randomly generated after $\wti{g}$ has been sent, \vs $$\wti{d}\equiv \slim_{i=1}^n d_i^p\gamma^i\Mod{\ell}\;\;\impop\;\;\wti{d} = \slim_{i=1}^n d_i^p\gamma^i.$$ Since $p> n\lam$, Lemma 2.3 now implies that with \op, $d_i\in\bz\;\forall\;i$

Now, an extractor $\E$ can query for a polynomial number of accepting transcripts, keeping the challenge $\gamma$ fixed and sampling the primes $\ell_j$, $\gamma_{0, j}$ randomly from the set of $\lam$-bit primes. $\E$ then uses the Chinese remainder theorem to compute $\mbf{e} = (e_1,\cdots,e_n)\in \bz^n$ such that $\mbf{e}\equiv \mbf{r}_j\Mod{\ell_j}$, where $\mbf{r}_j \in [\ell_j]^n$ is the vector of remainders modulo $\ell_j$ sent by $\A$. Furthermore, $e_i\leq 2^{q}$ where $q$ is the total number of queries that $\A$ makes to the group oracle $\mc{O}_2$. Since a single round of interaction with $\A$ results in an accepting transcript with probability $\epsilon\geq 1/\ttt{poly}(\lam)$, the extractor outputs the vector $(d_1,\cdots,d_n)$ in expected polynomial time. \end{prf}
...
\noin 7. $\mP$ computes the integers $q_0, r_0$ such that \vs $$\slim_{i=1}^n d_i\gamma_{0}^i = q_0\ell+r_0\;,\;r_0\in[\ell] .$$ He computes $Q_0:= a^{q_{0}}$ and sends $Q_0$ to $\V$.

\noindent 8. $\mc{V}$ verifies that $(r_1,\cdots,r_n)\in [\ell]^n$ and independently computes \vs $$b := \pl_{i=1}^n b_i^{\gamma^i}\;,\;b_0 := \pl_{i=1}^n b_i^{\gamma_{0}^i}\;\in \;\bG,$$ \vspace{-0.2cm} $$\wti{r}:= \slim_{i=1}^n r_i^{p}\gamma^i\Mod{\ell}\;,\;r:= \slim_{i=1}^n r_i\gamma^i\Mod{\ell}\;,\;r_0:=\slim_{i=1}^n r_i\gamma_{0}^i\Mod{\ell}.$$ 
...
\begin{prf} We first show that the protocol is sound, i.e. in case of an accepting transcript, the Prover possesses integers $d_i$ such that $$a^{d_i} = b_i\;\forall\; i\;\;\bigwedge\;\;f_j(d_1,\cdots,d_n) = 0\;\forall\; j $$ except with \np. We then show that an extractor $\E$ can efficiently extract the integers $d_i$ from sufficiently many accepting transcripts using the Chinese remainder theorem.

Since the equation $Q_0^{\ell} a^{r_0} = \pl_{i=1}^n b_i^{\gamma_{0}^i}$ holds, lemma 2.5 implies that with \op, the Prover possesses rationals $d_1,\cdots,d_n$ such that: \vs $$a^{d_i} = b_i \text{ for } i=1,\cdots,n \;\;\;\bigwedge\;\;\; \sum\limits_{i=1}^n d_i\gamma_{0}^i\equiv \sum\limits_{i=1}^n r_i\gamma_{0}^i\Mod{\ell}.$$ The low order assumption implies that the vector $\mbf{d}:= (d_1,\cdots, d_n)\;\in\;\bq^n$ is unique except with \np.

Since the challenge $\gamma_{0}$ is randomly generated after the Prover has sent $(r_1,\cdots,r_n)$, $\gamma_{0}$ is randomly and uniformly distributed modulo $\ell$. Hence, the Schwart-Zippel lemma implies that with \op, $d_i\equiv r_i\Mod{\ell}$ for every index $i$. Now, since the $\lam$-bit prime $\ell$ is randomly generated, \vs $$\Prob\left(f_j(d_1,\cdots,d_n)\equiv 0\Mod{\ell} \;\Big|\; f_j(d_1,\cdots,d_n)\neq 0 \right) = \negl(\lam) .$$ Hence, $$f_j(d_1,\cdots,d_n)\equiv 0\Mod{\ell}\;\forall\;j\;\impop\; f_j(d_1,\cdots,d_n) = 0 \;\forall\;j.$$ It remains to argue that the rationals $d_i$ are integers. The Verifier independently computes $$\wti{r}:= \slim_{i=1}^n r_i^{p}\gamma^i\equiv \slim_{i=1}^n d_i^{p}\gamma^i\Mod{\ell}.$$ So, the equation $\wti{g} = (\weck{g})^{\ell}g^{\wti{r}} $ and lemma 2.4 imply that the Prover possesses a rational $\wti{d}$ such that \vs $$\wti{g} = g^{\wti{d}}\;\;,\;\;\wti{d}\equiv \wti{r}\equiv \slim_{i=1}^n r_i^p\gamma^i\equiv \slim_{i=1}^n d_i^p\gamma^i\Mod{\ell}.$$ If $\wti{d}\notin\bz$, the tuple $(g, \wti{g}, \wti{d})$ would violate the fractional root assumption. So $\wti{d}$ is an integer, except with negligible probability. Since the $\lam$-bit prime $\ell$ is randomly generated after $\wti{g}$ has been sent by the Prover, the congruence \vs $$\wti{d}\equiv \slim_{i=1}^n d_i^p\gamma^i\Mod{\ell} $$ implies that with \op, $\wti{d} = \slim_{i=1}^n d_i^p\gamma^i.$ Hence, $\slim_{i=1}^n d_i^p\gamma^i\in \bz$ and Lemma 2.3 then implies that $ d_i\in \bz\;\forall\;i$ except with negligible probability. 

Now, an extractor $\E$ can query for a polynomial number of accepting transcripts, keeping the challenge $\gamma$ fixed and sampling the $\lam$-bit primes $\ell$, $\gamma_{0}$ randomly and uniformly from the set of $\lam$-bit primes. $\E$ makes $q$ queries, where $q$ is the total number of queries that $\A$ makes to the group oracle $\mc{O}_2$. Let $\ell_1,\cdots,\ell_q$ denote the primes in these queries and let $\mbf{r}_j\in [\ell_j]^n$ be the vector of remainders modulo $\ell_j$ in the accepting transcript. $\E$ uses the Chinese remainder theorem to efficiently compute $\mbf{e} = (e_1,\cdots,e_n)\in \bz^n$ such that \vs $$\mbf{e}\equiv \mbf{r}_j\Mod{\ell_j}\;\forall\; j\;\;,\;\;e_i\leq \pl_{i=k}^q \ell_j\;\forall\; i.$$ After making more than $q$ queries, the extractor obtains the vector $(d_1,\cdots,d_n)$ with \op.\end{prf} 
...
\noin 11. $\mc{V}$ verifies that $(\what{r}_1,\cdots,\what{r}_n, R)\in [\ell]^{n+1}$ and independently computes $r_i\equiv \what{r}_i^{-1}R\Mod{\ell}$ ($i=1,\cdots,n$) and \vs $$\what{r}:= \slim_{i=1}^n \what{r}_i\gamma^i\Mod{\ell}\;,\;\wti{r}:= {\slim_{i=1}^n r_i^{p}\gamma^i}\Mod{\ell}\;,\; \what{r}_0:= \slim_{i=1}^n \what{r}_i\gamma_{0}^i\Mod{\ell}$$

\noin 12. $\mc{V}$ verifies the equations \vs $$(\weck{a})^{\ell}a^R\sta A \; \;\bigwedge\;\; (\what{Q})^{\ell} a^{\what{r}}\sta \pl_{i=1}^{n} a_i^{\gamma^i}\;\;\bigwedge\;\;(\what{Q}_0)^{\ell}a^{\what{r}_0}\sta \pl_{i=1}^{n} a_i^{\gamma_{0}^i} \;\;\bigwedge\;\; (\weck{g})^{\ell}g^{\wti{r}} \sta \wti{g}.$$ He accepts the validity of the claim if and only if all equations hold and the proofs for \verb|PoKE|$[a,\;A]$, \verb|AggKE-1|$[a,\;(a_1,\cdots,a_n)]$ are valid.\qed \end{enumerate}
...
\begin{prf} (Sketch) We first show that the protocol is sound, i.e. in case of an accepting transcript, the Prover possesses integers $d_i$ such that $a_i^{d_i} = A\;\forall\; i$ except with \np. We then show that an extractor $\E$ can efficiently extract the integers $d_i$ from sufficiently many accepting transcripts using the Chinese remainder theorem.

The subprotocol \verb|PoAggKE-1|$[a,\;(a_1,\cdots,a_n)]$ demonstrates that with \op, the Prover possesses integers $\what{d}_1,\cdots,\what{d}_n$ such that \vs $$a_i = a^{\what{d}_i}\; (i=1.\cdots,n)$$ Furthermore, since the equation \vs $$\what{Q}_0^{\ell}a^{\what{r}_0} = \pl_{i=1}^{n} a_i^{\gamma_{0}^i} $$ holds, the adaptive root assumption implies that with \op, \vs $$\slim_{i=1}^n\what{r}_i\gamma_{0}^i\equiv \slim_{i=1}^n \what{d}_i\gamma_{0}^i\Mod{\ell}.$$ Since the $\lam$-bit challenge $\gamma_{0}$ is randomly generated, the Schwartz-Zippel lemma implies that with \op, $\what{r}_i\equiv \what{d}_i\Mod{\ell}\;\forall\; i$. 

The equation $(\weck{a})^{\ell}a^R= A$ implies that with \op, the Prover possesses a rational $D\equiv R\Mod{\ell}$ such that $a^D = A$. Thus, with \op, the rationals $D\what{d_1}^{-1},\cdots,D\what{d}_n^{-1}$ satisfy \vs $$D\what{d_i}^{-1}\equiv R\what{r}_i^{-1}\Mod{\ell} \text{ for every }i.$$ This, in turn, implies that with \op, $a_i^{D\what{d_i}^{-1}} = A \text{ for every }i.$ Now, the Verifier independently computes the $\lam$-bit integers $$r_i:= R\what{r}_{i}^{-1}\equiv D\what{d}_i^{-1}\Mod{\ell}\;\;\;(i=1,\cdots,n)$$\vspace{-0.3cm} $$r:= \slim_{i=1}^n r_i\gamma^i\equiv \slim_{i=1}^n D\what{d}_i^{-1}\Mod{\ell}\;\;,\;\;\wti{r}:= {\slim_{i=1}^n r_i^{p}\gamma^i}\equiv {\slim_{i=1}^n (D\what{d}_i^{-1})^{p}\gamma^i}\Mod{\ell}.$$ Hence, the equation $(\weck{g})^{\ell}g^{\wti{r}} = \wti{g}$ implies that with \op, the Prover possesses a rational $\wti{d}$ such that $\wti{g} = g^{\wti{d}}$. The adaptive root assumption implies that with \op, \vs $$\wti{d}\equiv \wti{r}\equiv \slim_{i=1}^n r_i^{{p}}\gamma^i \equiv   \slim_{i=1}^n \big(D\what{d_i}^{-1}\big)^{^{p}}\gamma^i \Mod{\ell}$$ Since the $\lam$-bit prime $\ell$ is randomly generated after $\wti{g}$ has been sent by the Prover, it follows that with \op, \vs $$\wti{d} = \slim_{i=1}^n \big(D\what{d_i}^{-1}\big)^{^{p}}\gamma^i. $$ Now, if $\wti{d}$ were not an integer, the tuple $(g, \wti{g}, \wti{d})$ would violate the fractional root assumption. Thus, with \op, $\wti{d}$ is an integer. Since $p > n\lam$, Lemma 2.3 now implies that with \op, the rationals $D\what{d_i}^{-1}$ are integers.

Now, there exists an extractor $\E_1$ for the protocol \verb|PoAggKE-1|$[a,\;(a_1,\cdots,a_n)]$ that with \op, extracts integers $\what{e}_1,\cdots,\what{e}_n$ such that $a^{\what{e}_i} = a_i$ in polynomial time. Furthermore, the $\ttt{PoKE}$ extractor, using the equations $\weck{a}^{\ell} a^{R} = A$ for sufficiently many $\lam$-bit prime challenges $\ell$ can extract a rational $E$ such that $a^{E} = A$ in polynomial time. The extractor $\E$ simulates both of these processes and accepts if and only if the rationals $E\what{e}_i^{-1}$ are all integers.\end{prf}
...
\begin{Prop} The Protocol \verb|PoGCD| is an argument of knowledge for the relations $\mc{R}_{{\tt{GCD}}}$ in the generic group model.\end{Prop}
...
\begin{prf} This is a special case of the preceding proposition.\end{prf}
...
\begin{prf} (Sketch) The subprotocol $\ttt{MultPolyDLog}[a,\;(a_1,\cdots,a_n,A, \what{A}),\; (f,\what{f})]$ implies that with \op,
the Prover possesses integers $D$, $\what{D}$, $d_1,\cdots,d_n$ such that \vs $$D = \pl_{i=1}^n d_i\;,\;\what{D} = \slim_{i=1}^n \frac{D}{d_i}   \;,\;a^{d_i} = a_i\;\forall\;i \;,\;a^{D} = A\;,\; a^{\what{D}} = \what{A} .$$ Furthermore, the subprotocol $\ttt{PoRelPrimeDLog}[(a,A),\;(a,\what{A})]$ implies that $\GCD(D,\what{D}) = 1$. Hence, by the preceding lemma, the integers $d_i$ are pairwise co-prime.\end{prf}



\bigskip

\noin Given elements $a_1, a_2\in \mb{G}$ and equations \vs $$a_1^{d_1} = b_1,\cdots, a_1^{d_m}= b_m\;\;,\;\; a_2^{e_1} = c_1,\cdots, a_2^{e_n} = c_n,$$ a Prover may provide a proof that he possesses the integers $d_1,\cdots, d_m,\;e_1,\cdots, e_n$ and that every pair $d_i, e_j$ is relatively prime. Clearly, the latter part is equivalent to the the integers $d:=\pl_{i=1}^m d_i$, $e:=\pl_{j=1}^n e_j$ being relatively prime. Our approach is to compute elements $B = a_1^d,\;C:= a_2^{e}$. We then  use the protocol \verb|RelPrimeDLog| to show that $\GCD(d, e) = 1$. Our primary use case is demonstrating the disjointness of two families of accumulators.
...
\noindent \textbf{Inputs:} Elements $a_1, a_2\in \mb{G}$; Elements $\mbf{b} = (b_1,\cdots, b_m)$, $\;\mbf{c} = (c_1,\cdots, c_n)$ of $\mb{G}^n$.
...
\noin Note that the protocol $\ttt{PoMultPolyDLog}[a_1,\;(b_1,\cdots,b_m, B),\; (\pl_{i=1}^m X_i) -X_{m+1}]$ contains\\ \verb|AggKE-1|$[a_1,\;\mbf{b}]$ as a subprotocol. Similarly, $\ttt{PoMultPolyDLog}[a_2,\;(c_1,\cdots,c_n, C),\; (\pl_{j=1}^n X_i) -X_{n+1}]$ contains \verb|AggKE-1|$[a_2,\;\mbf{c}]$ as a subprotocol. Thus, the proof consists of a constant number of $\mb{G}$-elements and $2(m+n)+\bO(1)$ $\lam$-bit integers.\vspace{0.2cm}
...
\begin{prf} The subprotocols \vs $$\ttt{PoMultPolyDLog}[a_1,\;(b_1,\cdots,b_m, B),\; f_1]\;\bigwedge\;\ttt{PoMultPolyDLog}[a_2,\;(c_1,\cdots,c_n, C),\; f_2] $$ imply that with \op, the Prover possesses integers $d_1,\cdots,d_m$, $e_1,\cdots,e_n$ such that \vs $$a_1^{d_i} = b_i\;,\;a_2^{e_j} = c_j \;,\; a_1^{\pl_{i=1}^m d_i} = B\;,\;a_2^{\pl_{j=1}^n e_j} = C \;,\;\;1\leq i\leq m\;,\;1\leq j\leq n.$$ Furthermore, the subprotocol \verb|PoRelPrimeDLog|$[(a_1, B),\;(a_2, C)]$ implies that with \op, $\GCD(\pl_{i=1}^m d_i, \pl_{j=1}^n e_j) = 1$.\end{prf}




\subsection{\fontsize{11}{11} Protocols for disjointness of sets/multisets in a single accumulator} 
...
\noin 11. $\mP$ computes \vs $$ \what{Q}:= w^{\what{q}}\;,\;\weck{g}:= g^{\wti{q}}\;\in\;\bG$$ and sends $\what{Q}, \weck{g}$ to $\V$.

\noin 12. The Fiat-Shamir heuristic generates a $\lam$-bit prime $\gamma_{0}$.

\noin 13. $\mP$ computes the integers $\what{q}_0, \what{r}_0$ such that \vs $$\slim_{i=1}^n \what{d}_i\gamma^i = \what{q}_0 \ell+\what{r}_0\;\;,\;\;\what{r}_0\in [\ell]$$ He computes $\what{Q}_0:= w^{\what{q}_0} \;\in\;\bG$ and sends it to $\V$.

\noin 14. $\V$ verifies that $(r_1,\cdots,r_n)\in [\ell]^{n}$ and independently computes the $\lam$-bit integers \vs $$R:= \pl_{i=1}^n r_i\Mod{\ell} \;\;,\;\;\what{r}_i = Rr_i^{-1}\Mod{\ell}\;\;(i=1,\cdots,n),$$\vspace{-0.3cm} $$\wti{r}:= \slim_{i=1}^n {r}_i^{p}\gamma^i\Mod{\ell} \;,\; \what{r}:= \slim_{i=1}^n \what{r}_i\gamma^i\Mod{\ell}\;,\;\what{r}_0:= \slim_{i=1}^n \what{r}_i\gamma_{0}^i\Mod{\ell}.$$

\noin 15. $\V$ verifies the equations \vs $$(\what{Q})^{\ell}w^{\what{r}}\sta \pl_{i=1}^n w_i^{\gamma^i} \;\bigwedge\; (\what{Q}_0)^{\ell}w^{\what{r}_0}\sta \pl_{i=1}^n w_i^{\gamma_{0}^i}\;\bigwedge\; A_0\cdot W\sta w\;\bigwedge\; (\weck{w})^{\ell}w^R\sta A\;\bigwedge\; (\weck{g})^{\ell}g^{\wti{r}}\sta\wti{g}$$ and the two \verb|PoKE|s from Step 4. He accepts if and only if all five equations hold and the two \verb|PoKE|s are valid. \qed \end{enumerate}
...
\begin{prf} We first show that the protocol is sound, i.e. in case of an accepting transcript, the Prover possesses integers $d_i$ such that \vs $$w_i^{d_i} = A\;\forall\; i\;\;\bigwedge\;\;\GCD(d_i,d_j) = 1\;\forall\; i,j $$ except with \np. We then show that an extractor $\E$ can efficiently extract the integers $d_i$ from sufficiently many accepting transcripts using the Chinese remainder theorem.

Since the challenge $\gamma_{0}$ is generated after the Prover sends $(r_1,\cdots,r_n)$, the equation \vs $$(\what{Q}_0)^{\ell}w^{\what{r}_0} = \pl_{i=1}^n w_i^{\gamma_{0}^i}, $$ the Schwartz-Zippel lemma and the adaptive root assumption imply that with \op, the Prover possesses rationals $\what{d}_1,\cdots,\what{d}_n$ such that $w^{d_i} = w_i\;,\;\what{d}_i\equiv \what{r}_i\Mod{\ell}$ for every $i$. Furthermore, the equation $(\weck{w})^{\ell}w^R = A$ implies that with \op, the Prover possesses a rational $D$ such that $D\equiv R\Mod{\ell}\;,\;w^{D} =A$. Now, \vs $$D\equiv R\equiv \pl_{i=1}^n r_i \equiv \pl_{i=1}^n d_i\Mod{\ell}$$ and since the $\lam$-bit prime $\ell$ is randomly generated after the Prover sends $w$, it follows that with \op, $D = \pl_{i=1}^n D\what{d}_i^{-1}$. Setting $d_i:= D\what{d}_i^{-1}$ yields $w_i^{d_i} = A$, $D = \pl_{i=1}^n d_i$.  

The equation $(\weck{g})^{\ell}g^{\wti{r}}=\wti{g}$ implies that with \op, the Prover possesses a rational $\wti{d}$ such that \vs $$\wti{g} = g^{\wti{d}}\;\;,\;\; \wti{d}\equiv \wti{r}\equiv\slim_{i=1}^n r_i^p\gamma^i\equiv \slim_{i=1}^n d_i^p\gamma^i \Mod{\ell}.$$ Since the $\lam$-bit prime $\ell$ is randomly generated after $\wti{g}$ has been sent by the Prover, it follows that with \op, \vs $$\wti{d} = \slim_{i=1}^n d_i^p\gamma^i.$$ The fractional root assumption implies that $\wti{d}$ is an integer except with \np. So $\slim_{i=1}^n d_i^p\gamma^i\in \bz$ and lemma 2.4 implies that with \op, the $d_i$ are all integers.

Furthermore, since we have $A_0\cdot W = w$ and the proofs for \verb|PoKE|$[A,\;A_0]$ and \verb|PoKE|$[(\pl_{i=1}^n w_i),\; W]$, it follows that, in particular, $w$ is expressible as a product \vs $$w =\pl_{i=1}^n w_i^{x_i}\;,\;(x_1,\cdots,x_n)\in\bz^n.$$ for some vector $(x_1,\cdots,x_n)$ known to the Prover. Hence, \vs $$w^{\LCM(d_1,\cdots,d_n)} = A^k = w^{k\pl_{i=1}^n d_i}$$ for some integer $k$ known to the Prover. Now, the low order assumption implies that with \op, $\LCM(d_1,\cdots,d_n)$ is divisible by the product $\pl_{i=1}^n d_i$, which forces equality between these two integers. Hence, the integers $d_i$ are pairwise co-prime.

Let $q$ be the total number of queries from $\A$ to the oracle $\mc{O}_2$. An extractor $\E$ can query $\A$ for $q$ accepting transcripts, keeping the challenge $\gamma$ fixed and sampling the challenges $\ell$, $\gamma_0$ randomly and uniformly from the set of $\lam$-bit primes. The extractor $\E$ samples $q$ challenges $\ell_1,\cdots,\ell_q$. Let $\mbf{r}_j\in [\ell_j]^n$ denote the vector of $n$ remainders modulo $\ell_j$ sent by $\A$. $\E$ uses the Chinese remainder theorem to obtain $\mbf{e} = (e_1,\cdots,e_n)\in\bz^n$ such that $\mbf{e}\equiv \mbf{r}_j \Mod{\ell_j}$ for each $j$. Now, with \op, $w_i^{e_i}  = A$ for every $i$ and the low order assumption implies that $e_i = d_i$ except with \np. \end{prf}
...
\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item Using Shamir's trick, $\mP$ computes elements $b, c \in \mb{G}$ such that \vs $$b^{\LCM(d_1,\cdots,d_m)} = B\;\; , \;\; c^{\LCM(e_1,\cdots,e_n)} = C$$ and sends $b,c$ to the Verifier $\mc{V}$.
...
\noindent 3. $\mc{P}$ generates non-interactive proofs for \verb|AggKE-1|$[b,\;\mbf{b}]$ and \verb|AggKE-1|$[c,\; \mbf{c}]$ and sends them to $\mc{V}$.
...
\begin{prf} (Sketch) The proofs for \verb|AggKE-2|$[\mbf{b}, B]$ and \verb|AggKE-1|$[b,\mbf{b}]$ imply that, with overwhelming probability, the Prover possesses integers $d$, $d_1,\cdots,d_m$, $\what{d}_1,\cdots,\what{d}_m$ such that \vs $$b^d = B\;,\; b^{\what{d}_i}= b_i\;,\; b_i^{d_i}= B\;\forall\; i.$$ Similarly, the proofs for \verb|AggKE-2|$[\mbf{c},\; C]$ and \verb|AggKE-1|$[c,\;\mbf{c}]$ imply that with overwhelming probability, the Prover possesses integers $e$, $e_1,\cdots,e_n$, $\what{e}_1,\cdots,\what{e}_n$ such that \vs $$c^e = C\;,\; c^{\what{e}_j}= c_j\;,\;c_j^{e_j}= C\;\forall\; j.$$ The low order assumption implies that \vs $$d_i\what{d}_i = d\;\forall\;i\;\;,\;\; e_i\what{e}_j = e\;\forall\;j$$ and in particular, $d$, $e$ are divisible by $\LCM(d_1,\cdots,d_m)$, $\LCM(e_1,\cdots,e_n)$ respectively. Lastly, the proof for \verb|RelPrimeDLog|$[(b,B),\;(c,C)]$ implies that with \op, $\GCD(d,e) = 1$. Hence, the integers $\LCM(d_1,\cdots,d_m)$, $\LCM(e_1,\cdots,e_n)$ are relatively prime, which completes the proof of soundness.
...
\subsubsection{\fontsize{11}{11} Multiset unions }
...
\begin{enumerate}[wide, labelwidth=!, labelindent=0pt]\vs \item The Prover $\mP$ computes the integers \vspace{-0.25cm} $$d:= \Pi\Big(\bigcup\limits_{i=1}^n \mc{M}_i\Big)\;\;,\;\;\weck{d}_i:= \frac{d}{\Pi(\mcM_i)}\;\;(i=1,\cdots,n).$$
...
\begin{Prop} The Protocol for frequency of elements in multisets $1$ is secure in the generic group model. \end{Prop}
...
Furthermore, the subprotocol \verb|PoAggKE-2|$[(A_2,\cdots,A_n),\; \what{A}_1]$ implies that with \op, $\LCM\big(\Pi(\mcM_2),\cdots, \Pi(\mcM_n)\big)$ divides $\what{m}_1$. Hence, for any prime $p$ dividing $d$ and any index $i\geq 2$, \vs $$\mr{val}_p(\Pi(\mcM_i))\leq  \mr{val}_p(\what{m}_1) < \mr{val}_p(ed).$$ On the other hand, $ed$ divides $d_1$ and hence, $\mr{val}_p(ed)\leq \mr{val}_p(d_1)$. \end{prf}
...
\begin{Prop} The Protocol for frequency of elements in multisets $3$ is secure in the generic group model. \end{Prop}

\begin{prf} The subprotocols \vs $$\ttt{PoGCD}[(\weck{w}_1, A),\;(w_1,\wti{w}_1^{\Pi(\mc{D})}),\;(w_1,\wti{w}_1)]\;\;\bigwedge\;\;\ttt{PoE}[\wti{w}_1,\;\Pi(\mc{D}),\;\wti{w}_1^{\Pi(\mc{D})}] $$ imply that with \op, the Prover possesses integers $e, \what{m}_1$ such that \vs $$\what{w}_1^{\what{m}_1} = A\;,\;w_1^{e} = \wti{w}_1\;,\; \GCD(ed, \what{m}_1)  = e .$$ The subprotocol \verb|PoAggKE-1|$[\weck{w}_1,\;(w_2,\cdots,w_n)]$ implies that with \op, $\what{m}_1$ is divisible by $\LCM(\Pi(\mcM_2),\cdots,\Pi(\mcM_n))$. So, for any prime $p$ dividing $d$ and any index $i\geq 2$, we have \vs $$\mr{val}_p(\Pi(\mcM_i)) \leq \mr{val}_p(\what{m}_1) < \mr{val}_p(ed).$$ On the other hand, the subprotocol $\ttt{PoKE}[\wti{w}_1^{\Pi(\mc{D})},\;A]$ implies that with \op, $ed$ divides $\Pi(\mcM_1)$. Hence, $\mr{val}_p(ed)\leq \mr{val}_p(\Pi(\mcM_1))$. \end{prf}
...
\noindent [BFS19] B. B\"{u}nz, B. Fisch, A. Szepieniec, \href{https://eprint.iacr.org/2019/1229}{\textit{Transparent SNARKs from DARK Compilers}}, Cryptology ePrint Archive, Report 2019/1229, 2019.  \vspace{0.1cm}
...
\noin [BD94] J. Benaloh and M. de Mare. \textit{One-way accumulators: A decentralized alternative to digital sinatures} (extended abstract). In Tor Helleseth, editor, Eurocrypt'93, volume 765 of LNCS, pages
274{285. Springer, Heidelberg, May 1994.\vspace{0.1cm}
...
\noin [BS96] W. Bosma and P. Stevenhagen. \textit{On the computation of quadratic 2-class groups} In Journal de Theorie des Nombres, 1996. \vspace{0.1cm}
...
\noindent [CFGKN20] M. Campanelli, D. Fiore, N. Greco, D. Kolonelos, L. Nizzardo, \href{https://eprint.iacr.org/2020/149}{\textit{Vector Commitment Techniques and Applications to Verifiable Decentralized Storage}} \vspace{0.1cm}
...
\noindent [DGS20] S. Dobson, S. Galbraith, B. Smith, \href{https://eprint.iacr.org/2020/196}{\textit{Trustless Groups of Unknown Order with Hyperelliptic Curves}} \vspace{0.1cm}

\noin [DK02] I. Damgard and M. Koprowski. \textit{Generic lower bounds for root extraction and signature schemes in general groups}. In Lars R. Knudsen, editor, Eurocrypt 2002, volume 2332 of LNCS, pages 256-271. Springer, Heidelberg, April / May 2002.\vspace{0.1cm}
...
\noin [Sho97] V. Shoup, \textit{Lower bounds for discrete logarithms and related problems}. In Walter Fumy, editor, Eurocrypt'97, volume 1233 of LNCS, pages 256{266. Springer, Heidelberg, May 1997. \vspace{0.1cm}
...
\noindent [Th20] S. Thakur, \href{https://eprint.iacr.org/2020/348}{\textit{Constructing hidden order groups using genus three Jacobians}}, \vspace{0.1cm} 
...
New York City, NY\\
Email: stevethakur01@gmail.com \end{comment}
...
\section{\fontsize{11}{11} \selectfont Proof of Exponentiation}
...
\noin 2. The Prover $\mP$ computes integers $q_i$, $r_i$ such that \vs $$e_i = q_i\ell + r_i\;\;,\;\; r_i\in[\ell] .$$

\noin 3. $\mP$ computes \vs $$Q:= \pl_{i=1}^n a_i^{q_i}$$ and sends $Q$ to the Verifier $\V$.

\noin 4. $\V$ computes $r_i:= e_i\Mod{\ell}$ and verifies the equation \vs $$ Q^{\ell}\pl_{i=1}^n a_i^{r_i} \sta b $$ and accepts if and only if the equation holds. \qed  \end{enumerate}
...
\noin \textbf{Soundness of this protocol:} Suppose, by way of contradiction, the Prover is able to output such an element $Q$ despite the equation $ \pl_{i=1}^n a_i^{e_i} = b$ being false. With \op, $\mP$ is able to extract integers $\al_{i,j}\; (1\leq i \leq n\;,\;1\leq j \leq k)$, $\al_1,\dots, \al_k$ such that \vs $$b = \pl_{j=1}^k g_j^{\be_j}\;,\;a_i = \pl_{j=1}^k g_j^{\al_{i,j}} $$ where the elements $g_1,\cdots,g_k\in \bG$ are responses to oracle $\mc{O}_1$ queries. Since the equation $Q^{\ell}\pl_{i=1}^n a_i^{r_i} = b $ holds, we have $\be_j\equiv \slim_{i=1}^n e_i\al_{i, j}\Mod{\ell}\;\;\forall\; j.$ Since the $\lam$-bit prime $\ell$ was randomly generated, it follows that with \op, $ \be_j = \slim_{i=1}^n e_i\al_{i, j}\;\;\forall\; j$ and hence,\\ $b = \pl_{i=1}^n a_i^{e_i}$. \qed
...
\noindent 10. $\V$ verifies that $(r_1,\cdots,r_n)\in [\ell]^n$ and independently computes \vs $$\wti{r}:= \slim_{i=1}^n r_i^{p}\gamma^i \Mod{\ell}\;,\;r:= \slim_{i=1}^n r_i\gamma^i \Mod{\ell}\;,\;r_0:= \slim_{i=1}^n r_i\gamma_{0}^i \Mod{\ell}.$$

\noin 11. $\V$ independently computes the elements \vs $$B:= \prod\limits_{i=1}^n b_i^{\gamma^i},\;\;B_0:= \prod\limits_{i=1}^n b_i^{\gamma_{0}^i},\;\;   C:= \prod\limits_{i=1}^n c_i^{\gamma^i}\;\in\; \mb{G}.$$

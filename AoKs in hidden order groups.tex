...
For a multiset $\mc{M}$ represented by $\lam$-bit primes and a hidden order group $\bG$, a \textit{commitment to a multiset} $\mc{M}$ is a pair $(g, h)\in\bG^2$ such that $g^{\Pi(\mc{M})} = h$. 
...
\noin 6. $\mP$ generates a non-interactive proof for \verb|RelPrimeDLog|$[(a,A),\;(a,\what{A})]$ and sends it to $\V$.
...
\noindent 3. $\mc{P}$ generates non-interactive proofs for \vs $$\ttt{MultPolyDLog}[a_1,\;(b_1,\cdots,b_m, B),\; (\pl_{i=1}^m X_i) -X_{m+1}]$$ and sends it to $\mc{V}$.

\noin 4. $\mc{P}$ generates non-interactive proofs for \vs $$ \ttt{MultPolyDLog}[a_2,\;(c_1,\cdots,c_n, C),\; (\pl_{j=1}^n X_j) -X_{n+1}]$$ and sends it to $\mc{V}$.

\noindent 5. $\mc{P}$ generates non-interactive proofs for 
\verb|AggKE-1|$[a_1,\mc{B}]$ and \verb|AggKE-1|$[a_2,\mc{C}]$ and sends them to the Verifer.

\noindent 6. $\mc{P}$ generates a non-interactive proof for \verb|RelPrimeDLog|$[(a_1, B),\;(a_2, C)]$ and sends it to $\mc{V}$. 

\noindent 7. $\mc{V}$ accepts the validity of the claim if and only if all of these proofs are valid.\end{prf1}
...
\noindent 3. $\mc{P}$ generates non-interactive proofs for \verb|AggKE-2|$[\mc{B}, B]$ and \verb|AggKE-2|$[\mc{C}, C]$ and sends them to $\mc{V}$.
...
\noin \textbf{Multiset sums:} For multisets $\mc{M}_1,\cdots,\mc{M}_n$, the server node can verifiably send the client the commitment $A_{\mr{prod}}$ for the sum \vs $$\sum\limits_{i=1}^n \mc{M}_i = \{(\sum\limits_{i=1}^n \mul(\mc{M}_i, x))\times x:\; x\in\bigcup\limits_{i=1}^n \sett(\mc{M}_i)\} $$ using the Protocol \vs $$\protect{\verb|PoMultPolyDLog|}[(g, A_1),\cdots,(g,A_n), (g,A_{\mr{prod})},\;(\pl_{i=1}^{n}X_i)-X_{n+1})].$$ The proof consists of a constant number of $\mb{G}$-elements and $n$ $\lam$-bit integers.

\bigskip

\noin \textbf{Multiset differences:} For multisets $\mc{M}$, $\mc{N}$, the difference $\mc{M}\sm \mc{N} $ has commitment \vs $$\protect{\verb|Com|}(g, \mc{M}\sm \mc{N}) = g^{\frac{\Pi(\mcM)}{\Pi(\mcM\cap \mc{N})}} .$$ So the Prover can use the Protocols \verb|PoGCD| and \verb|PoMultPolyDLog| to veriably send the commitment to $\mc{M}\sm \mc{N} $. 
...
\noin \textbf{Inputs:} Commitments $A_i = \ttt{Com}(g, \mcM_i) := ( g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\mr{int}}\in\mb{G}$
...
\noin 2. $\mP$ generates non-interactive proof for $\pr{\ttt{AggKE-1}}[A_{\mr{int}},\;(A_1,\cdots,A_n)]$, $\ttt{PoKE}[g,\;A_{\intt}]$ and sends them to the Verifier $\V$.

\noin 3. $\mP$ samples a $\lam$-bit prime $\gamma$ and computes \vs $$ e:= \GCD(\pl_{i=1}^n d_i, \slim_{i=1}^n d_i\gamma^i).$$

\noin 4. $\mP$ computes \vs $$\wti{e}:= \GCD(e^{n\lam}, \slim_{i=1}^n d_i^{n\lam}\gamma^i) .$$ If $\wti{e}$ does not divide $e^{n\lam-1}$, $\mP$ returns to Step 3 and samples another $\gamma$.  

\noin 5. $\mP$ computes $A:= A_{\intt}^{\pl_{i=1}^n d_i}$ and sends $\wti{A}$ to $\V$ along with a non-interactive proof for\\ $\ttt{MultPolyDLog}[A_{\intt},\;(A_1,\cdots,A_n, A), f]$ where $$f(x_1,\cdots,X_n):= (\pl_{i=1}^n X_i)-X_{n+1}.$$

\noin 6. $\mP$ computes $A_0:= A_{\intt}^{e}$ and sends it to $\V$ along with a non-interactive proof for \\ $\ttt{PoGCD}[(A_{\intt}, \pl_{i=1}^n A_i^{\gamma^i}),\;(A_{\intt},A),\;(A_{\intt},A_0)]$.

\noin 7. $\mP$ computes $\wti{A}:= A_{\intt}^{\slim_{i=1}^n d_i^{n\lam}\gamma^i}$ and sends $\wti{A}$ to $\V$ along with a non-interactive proof for \\ $\ttt{MultPolyDLog}[A_{\intt},\;(A_1,\cdots,A_n, \wti{A}),\;\wti{f}]$ where \vs $$f(X_1,\cdots,X_{n+1}):= \big(\slim_{i=1}^n \gamma^i X_i^{n\lam}\big) - X_{n+1}.$$

\noin 8. $\mP$ computes $A_{\intt}^{e^{n\lam}}, A_{\intt}^{e^{n\lam-1}}$ and sends $A_{\intt}^{e^{n\lam}}, A_{\intt}^{e^{n\lam-1}}$ to $\V$ along with non-interactive proofs for $$\ttt{PolyDLog}[(A_{\intt}, A_0),\;(A_{\intt}, A_{\intt}^{e^{n\lam}}),\;X^{n\lam}]\;,\;\ttt{PolyDLog}[(A_{\intt}, A_0),\;(A_{\intt}, A_{\intt}^{e^{n\lam-1}}),\;X^{n\lam-1}].$$
 
\noin 9. $\mP$ computes $\wti{A}_0 := A_{\intt}^{\wti{e}}$ and sends $\wti{A}_0$ to $\V$ along with a non-interactive proof for \\ $\ttt{PoGCD}[(A_{\intt}, \wti{A}),\;(A_{\intt},A_{\intt}^{e^{n\lam}}),\;(A_{\intt},\wti{A}_0)]$.

\noin 10. $\mP$ generates a non-interactive proof for $\ttt{PoKE}[\wti{A}_0,\; A_{\intt}^{e^{n\lam-1}}] .$

\noin 11. $\V$ verifies all the proofs he receives and accepts if and only if they are all valid.\end{prf1}
...
\noin \textbf{Inputs:} Commitments $A_i = \ttt{Com}(g, \mc{M}_i) := g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\mr{uni}}\in\mb{G}$

\noin \textbf{Claim:} $A_{\mr{uni}} = \protect{\verb|Com|}(g, {\bigcup\limits_{i=1}^n} \mc{M}_i) := g^{\Pi({\bigcup\limits_{i=1}^n} \mc{M}_i)}$.

\begin{prf1} \normalfont \noin 1. The Prover $\mP$ computes the integers \vs $$\weck{d}_i:= \frac{\Pi({\bigcup\limits_{i=1}^n} \mc{M}_i)}{\Pi(\mc{\mcM_i})}.$$

\noin 2. $\mP$ generates a non-interactive proof for $\ttt{AggKE-2}[(A_1,\cdots,A_n),\;A_{\mr{uni}}]$ and sends it to the Verifier $\V$.

\noin 3. $\mP$ samples a $\lam$-bit prime $\gamma$ and computes \vs $$e:= \GCD(\weck{d}_1, \slim_{i=1}^n d_i\gamma^i) .$$

\noin 4. $\mP$ computes $$\what{e}:= \GCD(e^{n\lam}, \slim_{i=1}^n \weck{d}_i^{n\lam}\gamma^i) .$$ If $\what{e}$ does not divide $e^{n\lam-1}$, $\mP$ returns to Step 3 and samples another $\gamma$.

\noin 5. $\mP$ computes $\weck{A}:= g^{\slim_{i=1}^n \weck{d}_i\gamma^i}$ and sends it to $\V$ along with a non-interactive proof for\\ $\ttt{MultPolyDLog}[g, \;(A_1,\cdots,A_n, A, \weck{A}), \weck{f}]$ where \vs $$\weck{f}(X_1,\cdots,X_{n+2}):= \Big( X_{n+1}\cdot \slim_{i=1}^n \frac{\gamma^i}{X_i} \Big)- X_{n+2}.$$

\noin 6. $\mP$ computes $A_0:= g^{e}$ and sends $A_0$ to $\V$ along with a non-interactive proof for\\ $\ttt{PoGCD}[(g, \weck{A}),\;(g, A_{\mr{uni}}),\;(g, A_0)].$

\noin 7. $\mP$ computes $\wti{A}:= g^{\slim_{i=1}^n \weck{d}_i^{n\lam}\gamma^i}$ and sends $\wti{A}$ to $\V$ along with a non-interactive proof for $\ttt{MultPolyDLog}[g, \;(A_1,\cdots,A_n, A_{\mr{uni}}, \wti{A}), \wti{f}]$ where \vs $$\wti{f}(X_1,\cdots,X_{n+2}):= \Big( X_{n+1}^{n\lam}\cdot \slim_{i=1}^n \frac{\gamma^i}{X_i^{n\lam}} \Big)- X_{n+2}. $$

\noin 8. $\mP$ computes $g^{e^{n\lam}}, g^{e^{n\lam-1}}$ and sends $g^{e^{n\lam}}, g^{e^{n\lam-1}}$ to $\V$ along with non-interactive proofs for $$\ttt{PolyDLog}[(g, A_0),\;(g, g^{e^{n\lam}}),\;X^{n\lam}]\;,\;\ttt{PolyDLog}[(g, A_0),\;(g, g^{e^{n\lam-1}}),\;X^{n\lam-1}].$$

\noin 9. $\mP$ computes $\wti{A}_0 := g^{\wti{e}}$ and sends $\wti{A}_0$ to $\V$ along with a non-interactive proof for \\ $\ttt{PoGCD}[(g, \wti{A}),\;(g,g^{e^{n\lam}}),\;(g,\wti{A}_0)]$.

\noin 10. $\mP$ generates a non-interactive proof for $\ttt{PoKE}[\wti{A}_0,\; g^{e^{n\lam-1}}] .$ and sends it to $\V$.

\noin 11. $\V$ verifies all the proofs he receives and accepts if and only if they are all valid. \end{prf1}

\noin The proof consists of a constant number of $\bG$-elements and $\bO(n)$ $\lam$-bit integers. As was the case with the protocol for multiset intersections, the proof is simpler in special cases such as when the elements of every $\mcM_i$ are all primes $> 2^{\lam}$.
...
\begin{prf1}\noin \normalfont 1. The Prover $\mP$ computes \vs $$\what{A}_1:= g^{\Pi\left(\bigcup\limits_{i=2}^n \mc{M}_i\right)} $$ and sends it to the verifier $\V$ along with a non-interactive proof for \verb|AggKE-2|$[(A_2,\cdots,A_n), \what{A}_1]$.
...
\begin{prf1}\noin \normalfont 1. The Prover $\mP$ computes \vs $$B_1:= g^{\pl_{x\in\mc{D}} x^{\mr{mult(\mc{M}_1, x)}}}\;,\; B_2 = B_1^{\Pi(\mc{D})}.$$
...
\begin{prf1}\noin \normalfont 1. The Prover $\mP$ uses Shamir's trick to compute an element $\weck{w}_1\in \mb{G}$ such that \vs $$\weck{w}_1 ^{\Pi(\bigcup\limits_{i=2}^n \mc{M}_i)} = A $$ and sends it to the Verifier $\V$.
...
\begin{prf1}\noin \normalfont 1. The Prover $\mP$ computes an element $\what{w}_1\in\mb{G}$ such that \vs $$\what{w}_1^{\Pi(\bigcap\limits_{i=2}^n \mc{M}_i)}:= A$$ and sends $\what{w}_1$ to the Verifier $\V$ along with a non-interactive proof for \verb|AggKE-2|$[\{w_2,\cdots,w_n\},\;\what{w}_1]$.
...
\noin 6. $\mP$ computes \vs $$t:= \slim_{i=1}^n d_ie_i\Mod{l}$$ and an element $Q_3\in \bG$ such that $Q_3^lg^{t} = A_{\mr{int}}.$
...
\noin 2. $\mP$ generates a non-interactive proof for $\pr{\ttt{AggKE-1}}[A_{\mr{int}},\;(A_1,\cdots,A_n)]$ and sends it to the Verifier $\V$.
...
\noin 3. $\mP$ generates a non-interactive proof for $\ttt{AggKE-1}[A_{\mr{int}},\;(A_1,\cdots,A_n)]$ and sends it to $\V$.
...
\noin 5. $\mP$ generates a non-interactive proof for $\ttt{PoGCD}[(g, \wti{A}),\;(g, \pl_{i=1}^n A_i^{\gamma^i}),\;(g, A_{\mr{int}})]$ and sends it to $\V$.
...
\noin \textbf{Inputs:} Commitments $A_i := g^{\Pi(\mc{M}_i)}$ for multisets $\mc{M}_i$; an element $A_{\mr{uni}}\in\mb{G}$

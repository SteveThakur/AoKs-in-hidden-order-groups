...
\noin Following the terminology of [BBF19], the event $\ttt{DLOG}$ refers to a generic PPT algorithm $\mc{A}$ generating random elements $g_1,\cdots g_n$ by making queries to the oracle $\mc{O}_1$ and generating integers $x_1,\cdots,x_n$ such that \vs $$\pl_{i=1}^n g_i^{x_i} = 1 .$$ The adaptive root assumption implies that the the probability of this event is negligible. The next lemma follows directly from the techniques in the appendix of [BBF19]. We provide a proof for the reader's convenience.
...
Now, an extractor $\E$ can query for a polynomial number of accepting transcripts, keeping the challenge $\gamma$ fixed and sampling the $\lam$-bit primes $\ell$, $\gamma_{0}$ randomly and uniformly from the set of $\lam$-bit primes. $\E$ makes $q$ queries, where $q$ is the total number of queries that $\A$ makes to the group oracle $\mc{O}_2$. Let $\ell_1,\cdots,\ell_q$ denote the primes in these queries and let $\mbf{r}_j\in [\ell_j]^n$ be the vector of remainders modulo $\ell_j$ in the accepting transcript. $\E$ uses the Chinese remainder theorem to efficiently compute $\mbf{e} = (e_1,\cdots,e_n)\in \bz^n$ such that \vspace{-0.3cm} $$\mbf{e}\equiv \mbf{r}_j\Mod{\ell_j}\;\forall\; j\;\;,\;\;e_i\leq \pl_{i=k}^q \ell_j\;\forall\; i.$$ After making more than $q$ queries, the extractor obtains the vector $(d_1,\cdots,d_n)$ with \op.\end{prf} 
...
\begin{prf} An extractor $\E$ can simulate the extractors for the subprotocols \vs $$\ttt{PoMultPolyDLog}[a_1,\;(b_1,\cdots,b_m, B),\; f_1]\;\bigwedge\;\ttt{PoMultPolyDLog}[a_2,\;(c_1,\cdots,c_n, C),\; f_2] $$ to extract integers $d_1,\cdots,d_m$, $e_1,\cdots,e_n$ such that \vs $$a_1^{d_i} = b_i\;,\;a_2^{e_j} = c_j \;,\; a_1^{\pl_{i=1}^m d_i} = B\;,\;a_2^{\pl_{j=1}^n e_j} = C \;,\;\;1\leq i\leq m\;,\;1\leq j\leq n.$$ Furthermore, the subprotocol \verb|PoRelPrimeDLog|$[(a_1, B),\;(a_2, C)]$ and the low order assumption imply that with \op, $\GCD(\pl_{i=1}^m d_i, \pl_{j=1}^n e_j) = 1$. This, in turn, implies that $\GCD(d_i,e_j) = 1$ for every pair $i,j$.\end{prf}
...
\noin 4. $\mP$ computes $$A_0:= A^{e}\;,\;W := \big(\pl_{i=1}^n w_i\big)^{\what{e}}$$ and sends $A_0,W$ to $\V$ along with non-interactive proofs for \verb|PoKE|$[A,\; A_0]$ and \verb|PoKE|$[(\pl_{i=1}^n w_i),\; W]$.
...
\noin \textbf{Multiset sums:} For multisets $\mc{M}_1,\cdots,\mc{M}_n$, the server node can verifiably send the client the commitment $A_{\Pi}$ for the sum \vs $$\sum\limits_{i=1}^n \mc{M}_i = \Big\{\big(\sum\limits_{i=1}^n \mul(\mc{M}_i, x)\big)\times x:\; x\in\bigcup\limits_{i=1}^n \sett(\mc{M}_i)\Big\} $$ using the Protocol \vs $$\ttt{PoMultPolyDLog}[g,\; (A_1,\cdots,A_n,A_{\Pi}),\;\big(\pl_{i=1}^{n}X_i\big)-X_{n+1}].$$ The proof consists of a constant number of $\mb{G}$-elements and $2n$ $\lam$-bit integers.
